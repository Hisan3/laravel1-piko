...
<div id="details" class="clearfix">
    @php
        $country_id = $billing_info->country_id;
        $city_id = $billing_info->city_id;
    @endphp
    <div id="client">
    <div class="to">INVOICE TO:</div>
    <h2 class="name">{{ $billing_info->fname }} {{ $billing_info->lname }}</h2>
    <div class="address">{{ $billing_info->address }},{{ App\Models\Country::where('id', $country_id)->first()->country_id }},{{ $billing_info->zip }},{{ App\Models\City::where('id', $city_id)->first()->city_id }}</div>
    <div class="email"><a href="mailto:john@example.com">{{ $billing_info->email }}</a></div>
    </div>
    <div id="invoice">
    <h1>INVOICE: {{ $order_info->order_id }}</h1>
    <div class="date">Date of Invoice: {{ $order_info->created_at->format('d-m-Y') }}</div>
    </div>
</div>
...
<div>
    ...
    <tr>
        ...
        <td>Tk{{ $order_info->discount }}</td>
    </tr>
    <tr>
        <td colspan="2"></td>
        <td colspan="2">Delievery Charge</td>
        <td>Tk{{ $order_info->charge }}</td>
    </tr>
    <tr>
        <td colspan="2"></td>
        <td colspan="2">Grand Total</td>
        <td>Tk{{ $order_info->total }}</td>
    </tr>
    </tfoot>
</table>
</div>
